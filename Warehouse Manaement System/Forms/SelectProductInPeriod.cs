...
namespace WareHouse.Forms
{
	public partial class SelectProductInPeriod : Form
	{
		...
		private void AddProdBtn_Click(object sender, EventArgs e)
		{
			...
			{
				var pId = Db.Products.Where(p => p.Prod_Name == ProdNamesCombo.Text).Select(p => p.Prod_Id).FirstOrDefault();
		...
		{
			if (
				FromDateFld.Text != String.Empty && ToDateFld.Text != String.Empty
				)
			{
