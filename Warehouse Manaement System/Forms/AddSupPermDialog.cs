...
namespace WareHouse.Forms
{
	public partial class AddSupPermDialog : Form
	{
		...
		{
			if (ProdQtyFld.Text != String.Empty && ProdNamesCombo.Text != String.Empty && ProdExpiryFld.Text != String.Empty && ProdProddateFld.Text != String.Empty)
			{
		...
		{
				ProdProddateFld.Text = ProdExpiryFld.Text = ProdNamesCombo.Text = ProdQtyFld.Text = String.Empty;
		}

		private void SubmitSupPermbtn_Click(object sender, EventArgs e)
		{
			if(SupPermDateFld.Text != String.Empty && SuppliersNamesCombo.Text != String.Empty && StoresNamesCombo.Text != String.Empty)
			{
				if(Products.Count > 0)
				{
					...
					int? supPermId = Db.SupplyPermissions.Max(s => (int?)s.SupPerm_Id);
					int PermId = supPermId ?? default(int); ;
					this.Products.ToList().ForEach(p => p.SupPerm_Id = PermId);
					foreach (var prod in Products)
					{
						var sup_prod = Db.Supplied_Prod.Where(s => s.Prod_Id == prod.Prod_Id && s.SupPerm_Id == storeId).FirstOrDefault();
						if(sup_prod == null)
						{
			...
			{
				MessageBox.Show("Some Permission Fields Are Empty");
			}
		}
	}
}
