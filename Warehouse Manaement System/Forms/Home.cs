...
namespace WareHouse
{
	public partial class Home : Form
	{
		...
		private void StoreData()
		{
			...
			var stores = Db.Stores.Select(s => s).ToList();
			foreach (var store in stores)
			{
				string[] row =
				{
					...
					store.Store_Email,
					store.Store_Address,
					store.Staffs.Where(s => s.Stf_Id == store.Manager_Id).Select(s => s.Stf_Fname + " " + s.Stf_Lname).FirstOrDefault()
				};
		...
		{
				StorenameFld.Text = StoreMailFld.Text = StorePhoneFld.Text = StoreIdFld.Text = ManagerCombo.Text = StoreAddrsFld.Text = String.Empty;
		}
		...
		private void Button1_Click(object sender, EventArgs e)
		{
			if(StoreIdFld.Text != String.Empty)
			{
				...
				var store = Db.Stores.Where(s => s.Store_Id == id).FirstOrDefault();
			...
			{
				MessageBox.Show("Please, Select a Store to Deleted");
			}
		}

		private void ClearStoreFields()
		{
			StorenameFld.Text = StoreIdFld.Text = StorePhoneFld.Text = StoreMailFld.Text = ManagerCombo.Text = StoreAddrsFld.Text = String.Empty;
		}

		private void Update_Click(object sender, EventArgs e)
		{
			if (StoreAddrsFld.Text != String.Empty && StorenameFld.Text != String.Empty && StorePhoneFld.Text != String.Empty && StoreMailFld.Text != String.Empty && ManagerCombo.Text != String.Empty && StoreIdFld.Text != String.Empty)
			{
				int id = int.Parse(StoreIdFld.Text);
				var store = Db.Stores.Where(s => s.Store_Id == id).FirstOrDefault();
		...
		private void ProductsInfo()
		{
			...
			var prods = Db.Products.Select(p => p).ToList();
		...
		{
			var stores = Db.Stores.Select(s => s.Store_Name).Distinct().ToList();
			foreach (var store in stores)
			{
		...
		{
			if (ProdNameFld.Text != String.Empty && StoresnamesCombo.Text != String.Empty && ProdNameFld.Text != String.Empty && ProdQtyFld.Text != String.Empty)
			{
				try
				{
				...
				{
					MessageBox.Show("Model Year Should Be A Number");
				}

				var storeId = Db.Stores.Where(s => s.Store_Name == StoresnamesCombo.Text).Select(s => s.Store_Id).FirstOrDefault();
				try
				{
		...
		{
			ProdNameFld.Text = ProdIdFld.Text = StoresnamesCombo.Text = ProdNameFld.Text = ProdQtyFld.Text = String.Empty;
		}

		private void UpdateProdBtn_Click(object sender, EventArgs e)
		{
			if (ProdIdFld.Text != String.Empty && StoresnamesCombo.Text != String.Empty && ProdNameFld.Text != String.Empty && ProdQtyFld.Text != String.Empty && ProdNameFld.Text != String.Empty)
			{
				int id = int.Parse(ProdIdFld.Text);
				var prod = Db.Products.Where(p => p.Prod_Id == id).FirstOrDefault();
				prod.Prod_Name = ProdNameFld.Text;
				prod.Model_Year = short.Parse(ProdYearFld.Text);

				var oldStoreId = Db.Stocks.Where(s => s.Prod_Id == id).Select(s => s.Store_Id).FirstOrDefault();

				var newStoreId = Db.Stores.Where(s => s.Store_Name == StoresnamesCombo.Text).Select(s => s.Store_Id).FirstOrDefault();

				Stock stock = null;
				foreach (var s in Db.Stocks)
				{
					if (s.Store_Id == oldStoreId && s.Prod_Id == id )
					{
						stock = s;
					}
				}
		...
		private void ProdList_Click(object sender, EventArgs e)
		{
			if (ProdList.SelectedItems.Count > 0)
			{
				...
				StoresnamesCombo.Text = Db.Stocks.Where(s => s.Prod_Id == id).Select(s => s.Store.Store_Name).FirstOrDefault();
				ProdQtyFld.Text = Db.Stocks.Where(s => s.Prod_Id == id).Select(s => s.Qty).FirstOrDefault().ToString();
			}
		...
		private void button2_Click(object sender, EventArgs e)
		{
			...
			{
				int id = int.Parse(ProdIdFld.Text);
				var product = Db.Products.Where(p => p.Prod_Id == id).FirstOrDefault();
		...
		private void SupplierInfo()
		{
			...
			var sups = Db.Suppliers.Select(s => s).ToList();
		...
		{
			if (SupMailFld.Text != String.Empty && SupSiteFld.Text != String.Empty && SupNameFld.Text != String.Empty && SupFaxFld.Text != String.Empty && SupTeleFld.Text != String.Empty && SupPhoneFld.Text != String.Empty)
			{
		...
		{
			if (SupIdFld.Text != String.Empty && SupNameFld.Text != String.Empty && SupPhoneFld.Text != String.Empty && SupFaxFld.Text != String.Empty && SupTeleFld.Text != String.Empty && SupMailFld.Text != String.Empty && SupSiteFld.Text != String.Empty)
			{
		...
		private void DeleteSupBtn_Click(object sender, EventArgs e)
		{
			...
			{
				int id = int.Parse(SupIdFld.Text);
				var sup = Db.Suppliers.Where(s => s.Sup_Id == id).FirstOrDefault();
		...
		{
			SupIdFld.Text = SupFaxFld.Text = SupNameFld.Text = SupPhoneFld.Text = SupTeleFld.Text = SupSiteFld.Text =SupMailFld.Text = String.Empty;
		}

		private void SuppPermData()
		{
			SupPermList.Items.Clear();
			var sups = Db.SupplyPermissions.Select(s => s).ToList();
			foreach (var sup in sups)
			{
		...
		private void SupPermList_Click(object sender, EventArgs e)
		{
			if (SupPermList.SelectedItems.Count > 0)
			{
				...
				else
				{
					...
					var sup = Db.SupplyPermissions.Where(s => s.SupPerm_Id == supPermId).FirstOrDefault();
					Db.SupplyPermissions.Remove(sup);
					Db.SaveChanges();
					SuppPermData();
				}
			}
		...
		private void DismList_Click(object sender, EventArgs e)
		{
			if (DismList.SelectedItems.Count > 0)
			{
				...
				else
				{
					...
					var dism = Db.Dismissals.Where(s => s.Dism_Id == dismId).FirstOrDefault();
		...
		private void button3_Click(object sender, EventArgs e)
		{
			...
			{
				string storeName = Db.Stores.Where(s => s.Store_Id == id).Select(s => s.Store_Name).FirstOrDefault();
				repData += $"Store: {storeName}\nStocks:\n";
				var stocks = Db.Stocks.Where(s => s.Store_Id == id).ToList();
		...
		private void ProdPeriodBtn_Click(object sender, EventArgs e)
		{
			...
			foreach(var id in RepProductsId)
			{
				...
				var sups = Db.Supplied_Prod.Where(s => s.Prod_Id == id).ToList();
				var disms = Db.Dism_Prod.Where(s => s.Prod_Id == id).ToList();
				if(sups.Count > 0)
				{
					repData += $"  1- Supplies Permissions:\n    Permisson Id    Store    Supplier    Date\n";
					foreach (var sup in sups)
					{
						if (sup.SupplyPermission.SupPerm_Date > RepFromDate && sup.SupplyPermission.SupPerm_Date < RepToDate)
						{
							repData += $"{sup.SupPerm_Id}   {sup.SupplyPermission.Store.Store_Name}   {sup.SupplyPermission.Supplier.Sup_Name}    {sup.SupplyPermission.SupPerm_Date.Value.ToString("dd/MM/yyyy")}";
						}
					}
				}
				if (disms.Count > 0)
		...
		private void PassedProdBtn_Click(object sender, EventArgs e)
		{
			...
			{
				int period = ((DateTime.Now.Year - p.SupplyPermission.SupPerm_Date.Value.Year) * 12)
					+ DateTime.Now.Month - p.SupplyPermission.SupPerm_Date.Value.Month;
				if (period > RepTimePeriod)
				{
					repData += $"- {p.Product.Prod_Name}    {p.SupplyPermission.Store.Store_Name}    {p.SupplyPermission.SupPerm_Date.Value.ToString("dd/MM/yyyy")}\n";
				}
			}
			ConvertDataToPdfFile(repData);
		}
		...
		private void ExpirationBtn_Click(object sender, EventArgs e)
		{
			...
			foreach (var p in Db.Supplied_Prod)
			{
				...
				int restMth = ((expDate.Year - p.Production_Date.Value.Year) * 12) + expDate.Month - p.Production_Date.Value.Month;
				if (restMth == RepExpiration)
				{
					repData += $"- {p.Product.Prod_Name}    {p.SupplyPermission.Store.Store_Name}    {p.Production_Date.Value.ToString("dd/MM/yyyy")}   {p.Expiry}\n";
				}
			}
			ConvertDataToPdfFile(repData);
		}
		...
		{
			if (
					ProdQtyF.Text != String.Empty && ProdProddateFld.Text != String.Empty && ProdNamesCombo.Text != String.Empty && ProdExpiryFld.Text != String.Empty && TransSupCombo.Text != String.Empty
				)
			{
				try
				{
					TransProduct prod = new TransProduct()
					{
						...
						ProdId = Db.Supplied_Prod.Where(p => p.Product.Prod_Name == ProdNamesCombo.Text).Select(p => p.Product.Prod_Id).FirstOrDefault(),
						Exp = int.Parse(ProdExpiryFld.Text),
						Qty = int.Parse(ProdQtyF.Text),
						Supplier = Db.Suppliers.Where(s => s.Sup_Name == TransSupCombo.Text).FirstOrDefault()
					};
				...
				{
					MessageBox.Show("Quantity and Expiry Date must be Numbers");
					return;
				}
			}
			else
			{
				MessageBox.Show("Some Products' Fields Are Empty");
			}
		}

		private void ClearTransProdFields()
		{
			ProdQtyF.Text = ProdExpiryFld.Text = ProdNamesCombo.Text = ProdProddateFld.Text = String.Empty;
		}

		private void TransBtn_Click(object sender, EventArgs e)
		{
			if (FromStoresCombo.Text != String.Empty && ToStoresCombo.Text != String.Empty)
			{
		...
		private void ClearAllFields()
		{
			...
			ProdExpiryFld.Text = ProdQtyF.Text = ToStoresCombo.Text = TransSupCombo.Text = FromStoresCombo.Text = String.Empty;
		}

		private void TransComboBoxDta()
		{
		...
		{
			var fromStore = Db.Stores.Where(s => s.Store_Name == FromStoresCombo.Text).FirstOrDefault();
			var toStore = Db.Stores.Where(s => s.Store_Name == ToStoresCombo.Text).FirstOrDefault();
			foreach ( var prod in TransProducts)
			{
				var fromProd = Db.Supplied_Prod.Where(p => p.Prod_Id == prod.ProdId && p.Expiry == prod.Exp && DbFunctions.TruncateTime(p.Production_Date) == prod.ProdDate.Date && p.SupplyPermission.Store_Id == fromStore.Store_Id && p.SupplyPermission.Sup_Id == prod.Supplier.Sup_Id).FirstOrDefault();
				if (fromProd != null)
				{
					var toStock = Db.Stocks.Where(s => s.Store_Id == toStore.Store_Id && s.Prod_Id == prod.ProdId).FirstOrDefault();
					var fromStock = Db.Stocks.Where(s => s.Store_Id == fromStore.Store_Id && s.Prod_Id == prod.ProdId).FirstOrDefault();
					if (toStock != null)
					{
		...
		{
			var fromSupPermProd = Db.Supplied_Prod.Where(s => s.SupplyPermission.Store_Id == fromStore.Store_Id && s.Prod_Id == prod.ProdId && s.Expiry == prod.Exp && DbFunctions.TruncateTime(s.Production_Date) == prod.ProdDate.Date).FirstOrDefault();
			fromSupPermProd.Qty -= prod.Qty;
			SupplyPermission newSupPerm = new SupplyPermission()
			{
				Store_Id = toStore.Store_Id,
				Sup_Id = prod.Supplier.Sup_Id,
				SupPerm_Date = DateTime.Now
			};
			Db.SupplyPermissions.Add(newSupPerm);
			Db.SaveChanges();
		...
		private void AddProductToSupllied_Prod(TransProduct prod)
		{
			...
			{
				Prod_Id = prod.ProdId,
				SupPerm_Id = Db.SupplyPermissions.Select(x => x.SupPerm_Id).Max(),
		...
		{
			if (StorenameFld.Text != String.Empty && StorePhoneFld.Text != String.Empty && StoreMailFld.Text != String.Empty && StoreAddrsFld.Text != String.Empty && ManagerCombo.Text != String.Empty)
			{
				try
				{
				...
				{
					MessageBox.Show("Manager Field Should be a Nuber");
				}
				Db.SaveChanges();
				StoreData();
				ClearStoreInputFields();
