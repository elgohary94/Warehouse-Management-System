...
{
	public partial class AddDism : Form
	{
		...
		{
			if(ProdNamesCombo.Text != String.Empty && ProdQtyFld.Text != String.Empty)
			{
				...
				MessageBox.Show("The Product Was Added Succeffuly");
				ClearProdFields();
			}
			else
			{
		...
		{
			ProdNamesCombo.Text = ProdQtyFld.Text = String.Empty;
		}

		private void SubmitDismBtn_Click(object sender, EventArgs e)
		{
			if (StoresNamesCombo.Text != String.Empty && DismPermDateFld.Text != String.Empty && CustomersNamesCombo.Text != String.Empty)
			{
				if (Products.Count > 0)
				{
					...
					int cusId = Db.Customers.Where(c => c.Cus_Fname + " " + c.Cus_Lname == CustomersNamesCombo.Text).Select(c => c.Cus_Id).FirstOrDefault();
					...
					int? dismId = Db.Dismissals.Max(s => (int?)s.Dism_Id);
					int id = dismId ?? default(int);
					this.Products.ToList().ForEach(p => p.Dism_Id = id);
					foreach (var product in Products)
					{
						var dism_prod = Db.Dism_Prod.Where(d => d.Prod_Id == product.Prod_Id && d.Dism_Id == product.Dism_Id).FirstOrDefault();
						if (dism_prod == null)
						{
